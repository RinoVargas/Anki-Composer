from backend.anki.input.input_data import InputDataRecord, InputData, InputDataField
from backend.compose.deck_specification import DeckSpecificationField, DeckSpecification, DeckSpecificationFieldType
...
def __generate_audio_from_fields(record: InputDataRecord, spec: DeckSpecification,
    ...
    for spec_field in audio_spec_fields:
        data_field = record.find_field_by_name(spec_field.column_name)
...
def __add_spec_audio_fields(spec: DeckSpecification, spec_fields: list[DeckSpecificationField]):
    for spec_field in spec_fields:
        spec.fields.append(DeckSpecificationField(f"$_{spec_field.id}", f"$_{spec_field.column_name}"))
